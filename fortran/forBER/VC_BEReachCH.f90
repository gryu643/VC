program VC_BEReachCH
    ...
    double precision Eig_ave(1,Nsybl)
    ...
    !file open
    open (1, file='VC_BEReachCH(s32p2).csv', status='replace')
    open (2, file='Eig.csv', status='replace')
    ...
    do KEbN0=SEbN0, EEbN0, Step !Eb/N0 loop
        ...
        !                call decomp_zgeev(Nsybl,V,Eig)
            ...
            do i=1, Nsybl
                Eig_ave(1,i) = Eig_ave(1,i) + Eig(1,i)/dble(Nloop)
            end do
            ...
            do i=1, Nsybl
                if(RdatI(1,i)==TdatI(1,i)) then
                    Collect(1,i) = Collect(1,i) + 1
                elseif(RdatI(1,i)/=TdatI(1,i)) then
                    False(1,i) = False(1,i) + 1
                endif
        ...
        !各チャネルのBERに対応する固有値が、降順になるように並び替え
        do i=1, Nsybl/2
            TMP = BER(1,i)
            BER(1,i) = BER(1,Nsybl+1-i)
            BER(1,Nsybl+1-i) = TMP
        end do
